...
exports.getByCardSerial = async (req, res) => {
  /**
   * @swagger
   *
   * /user/card/{cardSerial}/{deviceId}:
   *   get:
   *     security:
   *      - auth: []
   *     description: Get user by card serial
   *     tags: [User]
   *     consumes:
   *       - application/x-www-form-urlencoded
   *     produces:
   ...
   *         type: string
   *       - name: deviceId
   *         description: Device Id to in which the rfid is being registered
   *         in: path
   *         required: false
   *         type: number
   *     responses:
  ...
  try {
    ...
    if (req.user.admin) {
      if (!req.params.deviceId) {
        throw {
          message: "deviceId is required in path params if user is admin",
          status: 400,
        };
      }

      if (isNaN(parseInt(req.params.deviceId))) {
        throw { message: "deviceId should be a numeric value", status: 400 };
      }

      const organizationDevice = await UserDeviceModel.getDeviceDetails(
        req.params.deviceId,
      );

      if (!organizationDevice) {
        throw { message: "No device found with mentioned id", status: 404 };
      }

      organizationId = organizationDevice.dataValues.orgId;
    }

    const user = await UserModel.findByCardSerial(
      req.params.cardSerial,
      organizationId,
    );

    if (user) return apiResponse.success(res, req, user);
    else return apiResponse.fail(res, "User not found");
  } catch (error) {
    return apiResponse.fail(res, error.message, 500);
  }
};
...
exports.updateUserCardSerial = async (req, res) => {
  /**
   * @swagger
   *
   * /user/card/{userId}/{cardSerial}:
   *   put:
   *     security:
   *       - auth: []
   *     description: Update user card serial
   *     tags: [User]
   *     consumes:
   *       - application/x-www-form-urlencoded
   *     produces:
   ...
   *         type: string
   *       - name: orgId
   *         description: Organization Id in whcih you need to search user
   *         in: path
   *         required: false
   *         type: number
   *       - name: cardSerial
  ...
  try {
    const userUpdate = {
      cardSerial: req.params.cardSerial,
    };
    ...
    if (req.user.admin) {
      if (!req.body.deviceId) {
        throw new Error("deviceId is required in body if user is admin");
      }

      if (isNaN(parseInt(req.body.deviceId))) {
        throw new Error("deviceId should be a numeric value");
      }
      ...
      if (!organizationDevice) {
        throw new Error("No device found with mentioned id");
      }

      organizationId = organizationDevice.dataValues.orgId;
    }

    await isOverWrite(req.params.cardSerial, organizationId);
    const user = await UserModel.findById(req.params.userId, organizationId);

    if (user && user.status != UsersStatus.dummy) {
      // if user is admin, he can update card serial of all organizational users
      // if user is not admin, he can update card serial of his own organization's users
      await UserModel.update_user(user.id, userUpdate);
      ...
      helper.mqtt_publish_message("d/all/users", message, false);
      ...
      return apiResponse.fail(res, "Operation can not be performed");
    }
  } catch (error) {
    let statusCode = 500;
    if (error.message == "User not found") statusCode = 404;

    return apiResponse.fail(res, error.message, statusCode);
  }
};
...
exports.removeCardSerial = async (req, res) => {
  /**
   * @swagger
   *
   * /user/{userId}/remove/cardSerial/{orgId}:
   *   put:
   *     security:
   *     - auth: []
   *     description: Remove the card serial of user
   *     tags: [User]
   *     consumes:
   *       - application/x-www-form-urlencoded
   *     produces:
  ...
  try {
    ...
    const isValidUser = await UserModel.findById(
      req.params.userId,
      organizationId,
    );

    if (isValidUser.status != 3) {
      await UserModel.update_user(req.params.userId, { cardSerial: null });

      helper.mqtt_publish_message(
        "d/all/users",
        { status: "updated", deviceId: 0 },
        false,
      );
      ...
      apiResponse.success(
        res,
        req,
        "Card serial of user has been removed successfully",
      );
    } else throw { message: "No user found with mentioned Id.", status: 404 };
  } catch (err) {
    return apiResponse.fail(res, err?.message || err, err?.status || 500);
  }
};
