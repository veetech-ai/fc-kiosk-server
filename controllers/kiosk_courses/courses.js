...
exports.link_course_to_device = async (req, res) => {
  /**
   * @swagger
   *
   * /kiosk-courses/link-device:
   *   post:
   *     security:
   *       - auth: []
   *     description: link to device (Only Admin).
   *     tags: [Kiosk-Courses]
   *     consumes:
   *       - application/x-www-form-urlencoded
   *     parameters:
   *       - name: deviceId
   *         description: deviceId to be attached
   *         in: formData
   *         required: true
   *         type: integer
   *       - name: courseId
   *         description: golf course id to be attached to
   *         in: formData
   *         required: true
   *         type: ineteger
   *     produces:
