...
const getCourseFromDbSpy = jest
  .spyOn(courseServices, "getCourseFromDb")
...
const get_holes_by_courseIdSpy = jest
  .spyOn(golfbertService, "get_holes_by_courseId")
  .mockImplementation(async (golfBertCourseId) => {
    return {
      resources: [
        {
          id: 35097,
...
const get_scorecard_by_courseIdSpy = jest
  .spyOn(golfbertService, "get_scorecard_by_courseId")
  .mockImplementation(async (golfBertCourseId) => {
    return {
      holeteeboxes: [
        {
...
describe("mobile application courses for game play", () => {
  ...
  describe("[GET] /courses/{courseId}", () => {
    ...
    it("should throw error if course's par information is not found on golfbert", async () => {
      ...
      get_holes_by_courseIdSpy
        .mockImplementation(golfbertService, "get_holes_by_courseId")
        .mockImplementation(async (golfBertCourseId) => {
          return {
            resources: [
              {
      ...
      get_scorecard_by_courseIdSpy.mockImplementation(
        async (golfBertCourseId) => {
