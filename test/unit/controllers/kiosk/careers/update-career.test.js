...
describe("PATCH /careers/:careerId", () => {
    ...
    it("should return 400 and validation errors for the corresponding required fields", async () => {
        ...
        const response = await makePatchApiRequest({ title: 1, type: 1, content: 1, timings: "abc", link: 123 }, careers.test.id);
    ...
    it("should return 404 if the test organization's customer tries to update the career of some other organization", async () => {
        ...
        const response = await makePatchApiRequest({}, careers.zong.id, testCustomerToken);
    ...
    it("should return 200 if the test organization's customer tries to update the career with empty body", async () => {
        ...
        const response = await makePatchApiRequest({}, careers.test.id, testCustomerToken);
    ...
    it("should give an error if test organization's customer tries to update the career's golf course", async () => {
        ...
        const response = await makePatchApiRequest({ gcId: courses.zong.id }, careers.test.id, testCustomerToken);
    ...
    it("should update the career successfully if test organization's customer tries to update the career of his/her own organization", async () => {
        ...
        const response = await makePatchApiRequest(newBody, careers.test.id, testCustomerToken);
        expect(response.body).toStrictEqual(expectedResponse);
        expect(response.statusCode).toBe(200);
        
        const updatedCareer = await CareersServices.findOneCareer({ id: careers.test.id })
        expect(updatedCareer).toEqual(expect.objectContaining({...newBody, timings: {startTime: "9:00", endTime: "5:00"}}));
        
        
        await CareersServices.updateCareerById(careers.test.id, {...commonCareerBody})
    });
    ...
    it("should update the career successfully if super admin tries to update any existing career of the existing organization", async () => {
        ...
        console.log(abc);
        const response = await makePatchApiRequest(newBody, careers.test.id, superAdminToken);
        expect(response.body).toStrictEqual(expectedResponse);
        expect(response.statusCode).toBe(200);
